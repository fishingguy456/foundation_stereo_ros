...
class TrtRunner(nn.Module):
  def __init__(self, args, feature_runner_engine_path, post_runner_engine_path):
    ...
    self.feature_engine = self.runtime.deserialize_cuda_engine(engine_data)
    if self.feature_engine is None:
      raise RuntimeError(
        f"Failed to deserialize TensorRT feature engine: {feature_runner_engine_path}. "
        "Engine likely incompatible with current TensorRT/CUDA/GPU or missing plugins. "
        "Re-export ONNX and rebuild engines on this machine."
      )
    self.feature_context = self.feature_engine.create_execution_context()
    if self.feature_context is None:
      raise RuntimeError(f"Failed to create execution context for feature engine: {feature_runner_engine_path}")

    with open(post_runner_engine_path, 'rb') as file:
      engine_data = file.read()
    self.post_engine = self.runtime.deserialize_cuda_engine(engine_data)
    if self.post_engine is None:
      raise RuntimeError(
        f"Failed to deserialize TensorRT post engine: {post_runner_engine_path}. "
        "Engine likely incompatible with current TensorRT/CUDA/GPU or missing plugins. "
        "Re-export ONNX and rebuild engines on this machine."
      )
